...
For i.i.d.\ $\mathcal D=\{x_1,\dots,x_n\}$:
$p(\mathcal D)=\prod_{i=1}^n p(x_i),\;
\log p(\mathcal D)=\sum_{i=1}^n \log p(x_i).$

\bulletPoint{Expectation / Variance / Covariance}:
...
\bulletPoint{Vector Random Variables}:
For $x=[x_1,\dots,x_n]^\top$,
$\mathbb E[x]=[\mathbb E[x_1],\dots,\mathbb E[x_n]]^\top,\;
\mathbb E[Ax]=A\mathbb E[x].$
$\mathrm{cov}(x)=\Sigma_{xx}=\mathbb E[(x-\mathbb E[x])(x-\mathbb E[x])^\top].$
$\mathrm{cov}(Ax)=A\,\mathrm{cov}(x)\,A^\top,\;
\mathrm{cov}(x,y)=\Sigma_{xy}=\mathbb E[(x-\mathbb E[x])(y-\mathbb E[y])^\top].$

\bulletPoint{Bernoulli Distribution}:
$X\sim \mathrm{Bern}(\theta),\; x\in\{0,1\},\;
p(x\mid \theta)=\theta^x(1-\theta)^{1-x}.$
$\mathbb E[X]=\theta,\; \mathrm{var}(X)=\theta(1-\theta).$

\bulletPoint{Uniform Distribution}:
$X\sim \mathrm{Unif}(a,b)$
$p(x\mid a,b)=\frac{1}{b-a}$ for $a\le x\le b$, else $0$.
$\mathbb E[X]=\frac{a+b}{2},\;
\mathrm{var}(X)=\frac{(b-a)^2}{12}.$

\bulletPoint{Exponential Distribution}:
$X\sim \mathrm{Exp}(\lambda),\;
p(x\mid \lambda)=\lambda e^{-\lambda x}$ for $x\ge 0$, else $0$.
$\mathbb E[X]=\frac{1}{\lambda},\;
\mathrm{var}(X)=\frac{1}{\lambda^2}.$

\bulletPoint{Binomial Distribution}:
$X\sim \mathrm{Bin}(n,\theta),\;
P(X=x)=\binom{n}{x}\theta^x(1-\theta)^{n-x}; \binom{n}{x} = \frac{n!}{x!(n-x)!},\; x=0,\dots,n.$
$\mathbb E[X]=n\theta,\; \mathrm{var}(X)=n\theta(1-\theta).$


\bulletPoint{Parametric Bayesian Model}:
Assume $x\sim p(x\mid \theta)$. Then
$p(\theta\mid x)=\frac{p(x\mid \theta)p(\theta)}{p(x)}
\propto p(x\mid \theta)\,p(\theta),$
where posterior $\propto$ likelihood $\times$ prior.
Normalization:
$p(x)=\int p(x\mid \theta)p(\theta)\,d\theta.$

\bulletPoint{Conjugate Prior}:
If $p(\theta)$ and $p(\theta\mid x)$ have the same form, then $p(\theta)$ is a conjugate prior for $p(x\mid \theta)$.
$\log p(\theta\mid x)=\log p(x\mid \theta)+\log p(\theta)+\mathrm{const}.$

\bulletPoint{Beta Distribution}:
$\theta\sim \mathrm{Beta}(a,b),\; a,b>0,\; \theta\in[0,1],$
...
\bulletPoint{Bernoulli + Beta}:\\
1. For $x\in\{0,1\}$,
{\color{blue}$p(\theta\mid x)\propto \mathrm{Bern}(x\mid \theta)\,\mathrm{Beta}(\theta\mid a,b)
\propto \theta^{x+a-1}(1-\theta)^{b-x}.$}\\
Hence
$\theta\mid x \sim \mathrm{Beta}(a+x,\; b+1-x).$\\
2. For i.i.d.\ $\mathcal D=\{x_1,\dots,x_n\}$,\\
$p(\mathcal{D}\mid \theta)=
\prod_{i=1}^n p(x_i\mid \theta)=
\prod_{i=1}^n \theta^{x_i}(1-\theta)^{1-x_i}.$

{\color{blue}$p(\theta\mid \mathcal{D})
\propto
p(\mathcal{D}\mid \theta)\,p(\theta)
\propto
	\theta^{\sum_{i=1}^n x_i + a - 1}
(1-\theta)^{\,n-\sum_{i=1}^n x_i + b - 1}.$}

\useshortskip\noindent\resizebox{\linewidth}{!}{$
{\color{RoyalPurple!80!black}
\theta\mid \mathcal D \sim
\mathrm{Beta}\!\left(a+\sum_{i=1}^n x_i,\; b+n-\sum_{i=1}^n x_i\right).}
$}\\
\useshortskip\noindent\resizebox{\linewidth}{!}{$
\textcolor{MidnightBlue}{
\bm{\log p(\mathcal D\mid \theta)=
\sum_{i=1}^n \bigl(x_i\log\theta+(1-x_i)\log(1-\theta)\bigr).}
}
$}

\bulletPoint{Categorical Distribution}:
$X\sim \mathrm{Cat}(\theta_1,\dots,\theta_K),\;
\theta_k\ge 0,\; \sum_{k=1}^K \theta_k=1,\;
P(X=k)=\theta_k.$
For i.i.d.\ $\mathcal D=\{x_1,\dots,x_n\}$ and
$N_k=\sum_{i=1}^n \mathbf 1\{x_i=k\}$,
{\color{blue}
$p(\mathcal D\mid \theta)=\prod_{k=1}^K \theta_k^{N_k}.$
}
